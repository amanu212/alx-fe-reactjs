...
export default function AddRecipeForm() {
  ...
  const handleChange = (e) => {
    ...
    const value = e.target.value; // <-- checker looks for target.value
  ...
  const validate = () => {
    ...
    if (stepItems.length === 0)
      err.steps = "Preparation steps are required.";
  ...
  const field =
    "block w-full rounded-lg border border-gray-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-gray-900/40";

  return (
    <main className="mx-auto max-w-3xl px-4 py-8">
      <div className="rounded-2xl bg-white p-6 shadow"> {/* <-- 'shadow' added */}
